//! Coordinate geometry and interaction rules for the editor.
...
impl ViewTransform {
    ...
    pub const fn source_to_view(self, point: (f32, f32)) -> (f32, f32) {
        (
...
#[derive(Clone, Debug)]
/// Stateful editor interaction and visual-state model.
...
impl EditorModel {
    ...
    /// Applies a pad gesture and returns its normalized event data.
    ...
    /// Applies host voice state to the visual model.
    pub fn apply_external_state(&mut self, s: VisualState) {
        ...
        let selector = if !self.gate {
            ...
            0.8_f32.mul_add(self.vowel, 0.2)
        };
        self.frame = selector
            .mul_add(29.0, 0.5)
            .min(29.0)
    ...
    /// Returns the current artwork frame index.
    pub const fn animation_frame(&self) -> usize {
        self.frame
    }
}
